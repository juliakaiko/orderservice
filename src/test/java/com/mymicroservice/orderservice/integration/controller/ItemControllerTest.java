...
import com.mymicroservice.orderservice.controller.ItemController;
...
public class ItemControllerTest {
    ...
    public void getItemById_ShouldReturnItemDto() throws Exception {
        log.info("▶ Running test: getItemById_ShouldReturnItemDto(), TEST_ITEM_ID={}", TEST_ITEM_ID);
        when(itemService.getItemById(TEST_ITEM_ID)).thenReturn(testItemDto);

        mockMvc.perform(get("/api/items/{id}", TEST_ITEM_ID))
    ...
    public void getItemById_ShouldReturnNotFound() throws Exception {
        log.info("▶ Running test: getItemById_ShouldReturnNotFound(), _TEST_ITEM_ID={}", TEST_ITEM_ID);
        when(itemService.getItemById(TEST_ITEM_ID)).thenReturn(null);
    ...
    public void updateItem_ShouldReturnUpdatedItemDto() throws Exception {
        ...
        when(itemService.updateItem(TEST_ITEM_ID, updatedDto)).thenReturn(updatedDto);

        mockMvc.perform(put("/api/items/{id}", TEST_ITEM_ID)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        verify(itemService).updateItem(TEST_ITEM_ID, updatedDto);
    }
    ...
    public void updateItem_ShouldReturnNotFound() throws Exception {
        ...
        when(itemService.updateItem(TEST_ITEM_ID, updatedDto))
                .thenThrow(new ItemNotFoundException("Item wasn't found with id " + TEST_ITEM_ID));
    ...
    public void deleteItem_ShouldReturnDeletedItemDto() throws Exception {
        log.info("▶ Running test: deleteItem_ShouldReturnDeletedItemDto, _TEST_ITEM_ID={}", TEST_ITEM_ID);
        when(itemService.deleteItem(TEST_ITEM_ID)).thenReturn(testItemDto);

        mockMvc.perform(delete("/api/items/{id}", TEST_ITEM_ID))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(TEST_ITEM_ID))
    ...
    public void deleteItem_ShouldReturnNotFound() throws Exception {
        log.info("▶ Running test: deleteItem_ShouldReturnNotFound, TEST_ITEM_ID={}", TEST_ITEM_ID);
        when(itemService.deleteItem(TEST_ITEM_ID)).thenReturn(null);
