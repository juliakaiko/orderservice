...
import com.mymicroservice.orderservice.controller.OrderItemController;
...
public class OrderItemControllerTest {
    ...
    void setUp() {
        ...
        testOrderItem.setId(TEST_ITEM_ID);
        testOrderItemDto = OrderItemMapper.INSTANCE.toDto(testOrderItem);
        testOrderItemDto.setOrderId(TEST_ORDER_UUID);
        testOrderItemDto.setItemId(1L);
    }

    @Test
    public void getOrderItemById_ShouldReturnOrderItemDto() throws Exception {
        log.info("▶ Running test: getOrderItemById_ShouldReturnOrderItemDto(), TEST_ITEM_ID={}", TEST_ITEM_ID);
        when(orderItemService.getOrderItemById(TEST_ITEM_ID)).thenReturn(testOrderItemDto);

        mockMvc.perform(get("/api/order-items/{id}", TEST_ITEM_ID))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(TEST_ITEM_ID));

        verify(orderItemService).getOrderItemById(TEST_ITEM_ID);
    }

    @Test
    public void getOrderItemById_ShouldReturnNotFound() throws Exception {
        log.info("▶ Running test: getOrderItemById_ShouldReturnNotFound(), TEST_ITEM_ID={}", TEST_ITEM_ID);
        when(orderItemService.getOrderItemById(TEST_ITEM_ID)).thenReturn(null);
    ...
    public void updateOrderItem_ShouldReturnUpdatedOrderItemDto() throws Exception {
        ...
        updatedDto.setId(2L);
        updatedDto.setItemId(2L);
        updatedDto.setOrderId(TEST_ORDER_UUID);
        ...
        when(orderItemService.updateOrderItem(TEST_ITEM_ID, updatedDto)).thenReturn(updatedDto);

        mockMvc.perform(put("/api/order-items/{id}", TEST_ITEM_ID)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        verify(orderItemService).updateOrderItem(TEST_ITEM_ID, updatedDto);
    }
    ...
    public void updateOrderItem_ShouldReturnNotFound() throws Exception {
        ...
        when(orderItemService.updateOrderItem(TEST_ITEM_ID, updatedDto))
                .thenThrow(new OrderItemNotFoundException("OrderItem wasn't found with id " + TEST_ITEM_ID));
    ...
    public void deleteOrderItem_ShouldReturnDeletedOrderItemDto() throws Exception {
        log.info("▶ Running test: deleteOrderItem_ShouldReturnDeletedOrderItemDto, TEST_ITEM_ID={}", TEST_ITEM_ID);
        when(orderItemService.deleteOrderItem(TEST_ITEM_ID)).thenReturn(testOrderItemDto);

        mockMvc.perform(delete("/api/order-items/{id}", TEST_ITEM_ID))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(TEST_ITEM_ID))
                .andExpect(jsonPath("$.quantity").value(testOrderItemDto.getQuantity()));

        verify(orderItemService).deleteOrderItem(TEST_ITEM_ID);
    }

    @Test
    public void deleteOrderItem_ShouldReturnNotFound() throws Exception {
        log.info("▶ Running test: deleteOrderItem_ShouldReturnNotFound, TEST_ITEM_ID={}", TEST_ITEM_ID);
        when(orderItemService.deleteOrderItem(TEST_ITEM_ID)).thenReturn(null);
    ...
    public void getOrderItemsIdIn_ShouldReturnOrderItemsForGivenIds() throws Exception {
        ...
        mockMvc.perform(get("/api/order-items/find-by-ids")
                        .param("ids", TEST_ITEM_ID.toString()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value(TEST_ITEM_ID));
    ...
    public void getAllOrderItemsWithPagination_ShouldReturnPaginatedOrderItems() throws Exception {
        ...
        mockMvc.perform(get("/api/order-items/paginated")
                        ...
                        .param("size", "10"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content[0].id").value(TEST_ITEM_ID));
